...
public:
   ...
   inline void insert(const unsigned char* key_begin, const std::size_t& length)
   {
      ...
      for (std::size_t i = 0; i < salt_.size(); ++i)
      {
        ...
        SHA256(key_begin, temp_length, hash_char);
        compute_indices_sha(hash_char, bit_index, bit);
        ...
        //compute_indices_sha(key_begin, length, bit_index, bit);
   ...
   inline void insert(const InputIterator begin, const InputIterator end){
    ...
    inline void insert_at_index(unsigned int idx)
   {
      std::size_t bit=0;
      bit = idx % bits_per_char;
      bit_table_[idx / bits_per_char] |= bit_mask[bit];
      ++inserted_element_count_;
   }
    inline unsigned long long int  get_inserted_elments(bloom_filter t_bf){
    return t_bf.inserted_element_count_;
  } 
   ...
   inline std::size_t Calculate_ones_in_bf(const bloom_filter& f){
...
protected:
   ...
   inline void compute_indices_sha(unsigned char* hash_char, std::size_t& bit_index, std::size_t& bit) const{

    // unsigned char hash_char[SHA256_DIGEST_LENGTH];

    // SHA256(begin, remaining_length, hash_char);

    BIGNUM *hash_num= BN_bin2bn(hash_char, SHA256_DIGEST_LENGTH, NULL);

    BN_ULONG res = BN_mod_word(hash_num, table_size_);

    bit_index=(size_t)res;
